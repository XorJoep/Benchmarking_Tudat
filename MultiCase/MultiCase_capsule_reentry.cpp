...
void getAndSaveSolution(boost::shared_ptr< TranslationalStatePropagatorSettings< double > > propagatorSettings,
    ...
    std::string case_name, std::string propagator, std::string integrator, int step_size_exp, std::string outputDirectory) {
	...
	input_output::writeDataMapToTextFile( integrationResult,
        case_name + "_" + propagator + "_" + integrator + "_E" + boost::lexical_cast< std::string >(step_size_exp) + ".dat",
	...
	input_output::writeDataMapToTextFile( dynamicsSimulator.getDependentVariableHistory( ),
                            "DepVar_" + case_name + "_" + propagator + "_" + integrator + "_E" + boost::lexical_cast< std::string >(step_size_exp) + ".dat",
                            outputDirectory,
                            "",
...
int main( )
{
	...
	bodiesToCreate.push_back( frameOriginBody );
	
	std::map< std::string, boost::shared_ptr< BodySettings > > bodySettings =
			getDefaultBodySettings( bodiesToCreate, simulationStartEpoch, simulationEndEpoch + simulationEndEpochExtend);
	bodySettings[ frameOriginBody ]->ephemerisSettings->resetFrameOrientation( orientation );
	bodySettings[ frameOriginBody ]->rotationModelSettings->resetOriginalFrame( orientation );
	...
	bodyMap[ satelliteName ]->setAerodynamicCoefficientInterface(
            unit_tests::getApolloCoefficientInterface( ) );
	bodyMap[ satelliteName ]->setConstantBodyMass( 5.0E3 );
    ...
    accelerations[ frameOriginBody ].push_back( boost::make_shared< SphericalHarmonicAccelerationSettings >( 4, 0 ) );
    accelerations[ frameOriginBody ].push_back( boost::make_shared< AccelerationSettings >( aerodynamic ) );
    ...
    std::cout << "accelerationMap " << satelliteName << " made.." << std::endl;


    // Define constant 30 degree angle of attack
    double constantAngleOfAttack = unit_conversions::convertDegreesToRadians(30.0);
    bodyMap.at( satelliteName )->getFlightConditions( )->getAerodynamicAngleCalculator( )->setOrientationAngleFunctions(
                boost::lambda::constant( constantAngleOfAttack ) );

    std::cout << "AngleOfAttack setup.." << std::endl;

	// Define list of dependent variables to save.
	std::vector< boost::shared_ptr< SingleDependentVariableSaveSettings > > dependentVariablesList;
	dependentVariablesList.push_back(
            boost::make_shared< SingleDependentVariableSaveSettings >( mach_number_dependent_variable, satelliteName ) );
	dependentVariablesList.push_back(
	            boost::make_shared< SingleDependentVariableSaveSettings >(
	                altitude_dependent_variable, satelliteName, frameOriginBody ) );
	dependentVariablesList.push_back(
	            boost::make_shared< SingleAccelerationDependentVariableSaveSettings >(
	                aerodynamic, satelliteName, frameOriginBody, 1 ) );
	dependentVariablesList.push_back(
	            boost::make_shared< SingleDependentVariableSaveSettings >(
               		aerodynamic_force_coefficients_dependent_variable, satelliteName ) );
    dependentVariablesList.push_back(
                boost::make_shared< SingleDependentVariableSaveSettings >(
                    body_fixed_airspeed_based_velocity_variable, satelliteName, frameOriginBody ) );
	...
	std::cout << "DependentVariables setup.." << std::endl;
	...
	std::cout << "PropagatorTermination setup.." << std::endl;
	...
	for(unsigned int i = 0; i <states.size(); i++) { // cases
		...
		case_name = states[i][ "name" ];
		std::cout << "Calculating state: " << i + 1 << " out of " << states.size() << " named: " << case_name <<std::endl;
		...
		for(unsigned int j = 0; j < propagators.size(); j++){ // nBodyStateDerivative -> Enum for propagators with intvalue
			...
			boost::shared_ptr< TranslationalStatePropagatorSettings< double > > propagatorSettings =
                    ...
                    ( centralBodies, accelerationModelMap, bodiesToPropagate, systemInitialState, simulationEndEpoch, translationalPropagatorTypes[propagators[j]], dependentVariablesToSave);
				...
				if ( integrators[k].compare( "Euler" ) == 0 ){ // Euler -> fixed
						...
						step_size_exp += step_size_exp_step)
					{
						...
						getAndSaveSolution(propagatorSettings, integratorSettings, bodyMap, bodiesToCreate,
                         case_name, propagators[j], integrators[k], step_size_exp, outputDirectory);
					}
				}
